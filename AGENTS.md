...
`@knorby/incidecoder-client` is a universal TypeScript scraper client for
[incidecoder.com](https://incidecoder.com/). It is built to run in React
...
- **Pagination is next-link driven.** List parsers extract the authoritative
  ...
  `ppage`/`uoffset`/`offset` params. INCIDecoder uses a different scheme per
...
- **Live tests** (`npm run test:live`) hit incidecoder.com
  (`INCIDECODER_LIVE=1`) as a markup-change tripwire; they are excluded from the
